...
void L3G4200D::Update() 
{
  ...
  unsigned long int delta_time = current_time - last_recorded_time;
  angle = Matrix(1,3);
  
  float prev_data_x = angle(0);
  float prev_data_y = angle(1);
  float prev_data_z = angle(2);
  
  // angular velocity
  data_ang_velocity(0) = ((((int)buff_[1]) << 8) | buff_[0]) * 0.00875 + _xoffset;
  data_ang_velocity(1) = ((((int)buff_[3]) << 8) | buff_[2]) * 0.00875 + _yoffset;
  data_ang_velocity(3) = ((((int)buff_[5]) << 8) | buff_[4]) * 0.00875 + _zoffset;
